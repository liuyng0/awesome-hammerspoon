...
if spoon.WinWin then
    ...
    cmodal:bind(hyper2, 'F', 'Enter full screen', function()
    ...
    cmodal:bind(hyper2, 'M', 'Center Half Width', function() spoon.WinWin:stash() spoon.WinWin:moveAndResize("centerHalfWidth") end)
...
if spoon.Screen then
    ...
    cmodal:bind('', 'S', "Swap Windows with next Screen", function()
                  spoon.Screen:swapWithNext()
                  spoon.ModalMgr:deactivate({"screenM"})
    end)
    cmodal:bind(hsscreenM_keys[1], 'S', "Switch to Screen", function()
                  spoon.Space:switchToScreen()
                  spoon.ModalMgr:deactivate({"screenM"})
    end)
    cmodal:bind(hsscreenM_keys[1], 'J', "Choose tile with SplitView", function()
                  spoon.SplitView:choose()
                  spoon.ModalMgr:deactivate({"screenM"})
    end)
    cmodal:bind(hsscreenM_keys[1], 'K', "Swap windows with SplitView", function()
                  spoon.SplitView:swapWindows()
                  spoon.ModalMgr:deactivate({"screenM"})
    end)
    cmodal:bind(hsscreenM_keys[1], 'L', "Switch Focus with SplitView", function()
                  spoon.SplitView:switchFocus()
                  spoon.ModalMgr:deactivate({"screenM"})
    end)
    cmodal:bind(hsscreenM_keys[1], ';', "Removal desktop with SplitView", function()
