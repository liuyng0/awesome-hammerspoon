...
local getSpacesIdsTable = function()
    ...
    hs.fnutils.each(
        ...
        function(screen)
            local spaceUUID = spaces.spaceDisplay(spaces.activeSpaceOnScreen(screen))
            local userSpaces =
                hs.fnutils.filter(
...
local getCurrentSpacesByScreen = function()
    local currentSpaces = spaces.activeSpaceOnScreen()
...
module.setLayout = function(layout, spaceId)
    ...
    cache.floating =
        ...
        cache.floating,
        function(win)
            return spaces.windowSpaces(win)[1] ~= spaceId
        end
    )
...
module.getLayout = function(spaceId)
    ...
    local screen =
        ...
        hs.screen.allScreens(),
        function(screen)
            return spaces.spaceDisplay(spaces.activeSpaceOnScreen(screen)) == foundScreenUUID
        end
    )
...
module.toggleFloat = function(win)
    ...
    if module.isFloating(win) then
        local spaceId = spaces.windowSpaces(win)[1]
        local foundIdx
        ...
        function(spaceId)
            ...
            for i = #spaceWindows, 1, -1 do
                ...
                local existsOnScreen =
                    ...
                    function(win)
                        local winSpaces = spaces.windowSpaces(win)
                        return win:id() == spaceWindows[i]:id() and winSpaces[1] == spaceId
                    end
                )
    ...
    if module.filters then
        local foundMatch =
            ...
            function(obj)
                ...
                local titleMatches =
                    ternary(obj.title ~= nil and title ~= nil, string.match(title, obj.title or ""), true)
...
module.start = function()
    ...
    -- start window filter
    cache.filter =
        hs.window.filter.new():setDefaultFilter():setOverrideFilter(
        {
