...
function obj:choose(winChoices)
    ...
    if winChoices then -- Make sure not to include focused win
        ...
        winChoices =
            hs.fnutils.filter(
            win:otherWindowsSameScreen(),
            function(w)
                return w:isStandard() and not w:isFullScreen()
            end
        )
    end
    ...
    for _, w in pairs(winChoices) do
        ...
        if self.showImage then
            choice.image = hs.window.snapshotForID(w:id())
        end
...
function obj:byName(otherapp, otherwin, noChoose)
    ...
    if otherapp then
        otherapp = hs.application.find(otherapp)
    end
    ...
    if self.debug then
        ...
        for i, v in ipairs(selectWins) do
            print(i, v)
        end
    end

    if not selectWins or #selectWins == 0 then
        return
    end
...
function obj:performSplit(thiswin, otherwin)
    if not thiswin or not otherwin or thiswin == otherwin then
        return
    end
    ...
    hst.waitUntil(
        ...
        function()
            ...
            hst.waitUntil(
                function()
                    return thiswin:isFullscreen()
                end, -- close but not really done
                function()
                    hst.doAfter(
                        self.delayOtherClick,
                        function()
...
function obj:findSafeDither(x, y, targ, targwin)
    ...
    if hitCnt == 8 then
        return x, y -- full coverage, take original
    ...
    elseif hitCnt == 3 then -- corner: take good interior corner
        for offx = -1, 1, 2 do
            for offy = -1, 1, 2 do -- take the good corner
...
function obj:findMiniSplitViewWindow(thiswin, targwin)
    ...
    if thiswin:frame().x == frame.x then
        targ.x = targ.x + frame.w / 2
    end -- search RHS
    ...
    if self.debug then
        print(string.format("SplitView: Tiling %d windows with initial %d x %d grid", wcnt, n, m))
    end
    ...
    while iter < self.maxRefineIter do
                ...
                for j = 1, m do
                    ...
                    if win and win == targwin then
                        if self.debug then
                            print(
                                string.format(
                                    "Searched %d locations, matched at x=%0.1f, y=%0.1f" .. " (jiggle %0.3f,%0.3f)",
                                    cnt,
                                    x,
                                    y,
                                    jiggle[1],
        ...
        if self.debug then
            print(string.format("Failed to match at jiggle %0f, increasing resolution by a factor of 2.", jigStep))
        end
        jigStep = jigStep / 2
        jiggleSet =
            hs.fnutils.mapCat(
            jiggleSet,
            -- offset and refine jiggle grid
            function(p)
                local t = {}
                for x = -1, 1, 2 do
                    for y = -1, 1, 2 do
...
function obj:switchFocus()
    local win = getGoodFocusedWindow()
    if not win or not win:isFullScreen() then
        return
    end
...
function obj:swapWindows()
    ...
    if not (win and win:isFullScreen()) then
        return
    end
    ...
    if not already_down then
        mm:location({x = x, y = ff.y}):post()
    end

    local oldy
    hst.waitUntil(
        function()
            -- wait until the clickPoint settles
            if alreadyDown then
                return true
            end
...
function obj:removeCurrentFullScreenDesktop()
    ...
    for _, s in pairs(hs.screen.allScreens()) do
        frame = s:fullFrame()
        if cursor.x >= frame.x and cursor.x < frame.x + frame.w and cursor.y >= frame.y and cursor.y < frame.y + frame.h then
    ...
    if self.debug then
        print("Space: ", space, " Type: ", type)
    end
    if not (type == "fullscreen" or type == "tiled") then
        return
    end
