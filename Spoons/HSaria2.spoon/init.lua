...
local function processRemainTime(completed, total, speed, status)
    local function formatTime(secnum)
        ...
        return string.format("%02d", hourcount) ..
...
function obj:createAndRefreshPanel()
    if obj.rpc_host and obj.rpc_secret then
        ...
        hs.http.asyncPost(
            obj.rpc_host,
            hs.json.encode(tasks_overview_req),
            nil,
            function(status, data)
                if status == 200 then
                    obj:drawToolbar()
                    ...
                    for idx, val in ipairs(decoded_data) do
                        if type(val.result) == "table" then
                            for _, k in ipairs(val.result) do
                                if obj.itemcount >= obj.max_items then
                                    break
                                end
                                ...
                                local remain_time =
                                    processRemainTime(k.completedLength, k.totalLength, k.downloadSpeed, k.status)
                                -- Create task items
                                ...
                                if k.status == "active" then
                                    aria2_taskitem[1] = {
                                        type = "rectangle",
                                        id = "background",
                                        fillColor = {hex = "#26C83F", alpha = 0.1}
                                    }
                                elseif k.status == "paused" then
                                    aria2_taskitem[1] = {
                                        type = "rectangle",
                                        id = "background",
                                        fillColor = {hex = "#FFBB2D", alpha = 0.1}
                                    }
                                elseif k.status == "complete" then
                                    aria2_taskitem[1] = {
                                        type = "rectangle",
                                        id = "background",
                                        fillColor = {hex = "#000000", alpha = 0.1}
                                    }
                                elseif k.status == "error" or k.status == "removed" then
                                    aria2_taskitem[1] = {
                                        type = "rectangle",
                                        id = "background",
                                        fillColor = {hex = "#FE534C", alpha = 0.1}
                                    }
                                end
                                aria2_taskitem[2] = {
                                    type = "text",
                                ...
                                aria2_taskitem[5] = {
                                    ...
                                    id = "progressbg",
                                    type = "rectangle",
                                    fillColor = {hex = "#F6F6F6", alpha = 0.3},
                                ...
                                aria2_taskitem[6] = {
                                    ...
                                    id = "progressfg",
                                    type = "rectangle",
                                    fillColor = {hex = "#1E90FF", alpha = 0.7},
                                    frame = {x = "2%", y = "55%", w = tostring(0.6 * download_progress), h = "25%"}
                                }
                                aria2_taskitem[7] = {
                                    type = "text",
                                    id = "dpercent",
                                ...
                                aria2_taskitem:mouseCallback(
                                    function(canvas, event, id, x, y)
                                        if canvas == aria2_taskitem and event == "mouseDown" then
                                            ...
                                            if modifiers_status.cmd == true then
                                                if
                                                    k.status == "complete" or k.status == "removed" or
                                                        k.status == "error"
                                                 then
                    ...
                    obj.panel:frame(
                        {
                            x = cres.x + cres.w - 400,
                            y = cres.y + cres.h - 50 * #obj.panelItems - 52,
                            w = 400,
                            h = 50 * #obj.panelItems
                        }
                    )
                    ...
                    for idx, val in ipairs(obj.panelItems) do
                        obj.panel[idx + 1] = {
                            ...
                            frame = {
                                x = "0%",
                                y = tostring(1 / #obj.panelItems * (idx - 1)),
                                w = "100%",
                                h = tostring(1 / #obj.panelItems)
                            }
                        }
                    end
                    -- TODO: Figure out why this is needed
                    obj.panel:mouseCallback(
                        function(canvas, event, id, x, y)
...
function obj:intervalRequest()
    ...
    if not obj.stopped_queue_cached then
    ...
    hs.http.asyncPost(
        ...
        function(status, data)
            if status == 200 then
                ...
                if not obj.global_stoppednum then
                    obj.global_stoppednum = stoppednum
                end
                if not obj.global_activenum then
                    obj.global_activenum = activenum
                end
                if not obj.global_waitingnum then
                ...
                if
                    activenum ~= obj.global_activenum or waitingnum ~= obj.global_waitingnum or
                        stoppednum ~= obj.global_stoppednum
                 then
                        ...
                        for idx, val in ipairs(decoded_data[2].result) do
                            if not isInStoppedQueue(val.gid, obj.stopped_queue_list) then
                                ...
                                elseif val.errorCode == "31" then
                                    ...
                                    hs.notify.new(
                                        {title = "Download Error! " .. val.errorMessage, informativeText = file_path}
                                    ):send()
                                end
                            end
                        end
                ...
                if obj.active_request_added then
                    ...
                    for i = obj.active_pos, #decoded_data do
                        ...
                        local download_progress =
                            ...
                            decoded_data[i].result.completedLength,
                            decoded_data[i].result.totalLength
                        )
                        local progress_percent = tostring(download_progress * 100) .. "%"
                        ...
                        obj.panelItems[i - (obj.active_pos - 1)].canvas[6].frame = {
                            x = "2%",
                            y = "55%",
                            w = tostring(0.6 * download_progress),
                            h = "25%"
                        }
...
function obj:drawToolbar()
    if not obj.toolbar then
        ...
        obj.toolbar[1] = {
            action = "fill",
            type = "rectangle",
            fillColor = {hex = "#20B2AA", alpha = 0.3},
        ...
        obj.toolbar[2] = {
            ...
            id = "regular",
            text = "➲",
            frame = {x = 0, y = 0, w = tostring(1 / 4), h = "100%"},
            textSize = 20,
            textAlignment = "center"
        }
        obj.toolbar[3] = {
            ...
            id = "bt",
            text = "❒",
            frame = {x = tostring(1 / 4), y = 0, w = tostring(1 / 4), h = "100%"},
            textSize = 20,
            textAlignment = "center"
        }
        obj.toolbar[4] = {
            ...
            id = "clear",
            text = "♻︎",
            frame = {x = tostring(2 / 4), y = 0, w = tostring(1 / 4), h = "100%"},
            textSize = 20,
            textAlignment = "center"
        }
        obj.toolbar[5] = {
            ...
            id = "close",
            text = "✖︎",
            frame = {x = tostring(3 / 4), y = 0, w = tostring(1 / 4), h = "100%"},
            textSize = 20,
            textAlignment = "center"
        }
        obj.toolbar._default.trackMouseDown = true

        obj.toolbar:mouseCallback(
            function(canvas, event, id, x, y)
