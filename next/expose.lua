...
local min, max, ceil, abs, fmod, floor, random = math.min, math.max, math.ceil, math.abs, math.fmod, math.floor,
    math.random
local next, type, ipairs, pairs, sformat, supper, ssub, tostring = next, type, ipairs, pairs, string.format, string
    .upper, string.sub, tostring
...
local function sortedWindows (t, comp)
  local r = {}
  for _, w in pairs(t) do r[#r + 1] = w end
  tsort(r, comp)
  return r
end
...
local function fitWindows (self, screen, maxIterations)
  ...
  if not haveThumbs then -- "fast" mode
    for _, w in pairs(windows) do
      if w.dirty then
        w.frame:setw(minWidth):seth(minHeight):setcenter(w.originalFrame.center):fit(screenFrame)
        w.ratio = 1
      end
      w.weight = 1 / nwindows
    end
    ...
    if isStrip and s < 1 then
      ...
      for _, w in pairs(windows) do
        if w.dirty then
          w.frame = geom.copy(w.originalFrame):scale(min(1, optimalRatio * 2))
          w.ratio = min(1, optimalRatio * 2)
        else
          w.ratio = w.frame.area / w.originalFrame.area
        end
        w.weight = w.originalFrame.area / screen.totalOriginalArea
  ...
  while didwork and iterations < maxIterations do
    ...
    for _, w in pairs(windows) do
      ...
      for i = MAXTEST, 1, -1 do
        local test = TESTFRAMES[i]
        ...
        if tarea > 0 then
          ...
          if ac.length > maxDisplace then
            ac.length = maxDisplace
            --          else
            --            ac:move(random(-10,10)/20,random(-10,10)/20)
            --             end
          elseif ac:floor() == VEC00 then
            ac:move(random(-10, 10) / 20, random(-10, 10) / 20)
          end
      ...
      w.frame = wframe
      w.ratio = wframe.area / w.originalFrame.area
    end
    ...
    if not didwork or halting then
      ...
      self.log.vf('%s: %s (%d iter), coverage %.2f%%, ratio %.2f%%/%.2f%%, %d overlaps', screen.name,
        didwork and 'halted' or 'optimal', iterations, totalArea / (screenFrame.area) * 100, totalRatio / nwindows * 100,
        optimalRatio * 100, totalOverlaps)
    ...
    for id, w in pairs(screen.windows) do
      if not windows or windows[id] then
        ...
        while #appname < self.ui.maxHintLetters do
          appname = appname .. tostring(reservedHint)
          reservedHint = reservedHint + 1
        end
        w.appname = appname
        hints[#hints + 1] = w
    ...
    elseif type(c) == 'string' and #c == 1 then
      ...
      if w then
        if ui.showThumbnails then
          if show then
            w.thumb:setAlpha(ui.highlightThumbnailAlpha)
            w.highlight:show()
          else
            w.thumb:setAlpha(ui.thumbnailAlpha)
            w.highlight:hide()
          end
        end
        ...
        if show then
          w.hintrect:show()
          w.curhint = ssub('        ', 1, #modals + (entering and 0 or -1)) .. ssub(w.hint, #modals +
            (entering and 1 or 0))
        ...
        updateHighlights(ui, t, subtree, entering, show)
      end
    end
  end
end
...
local function exitAll (self, windowSelected)
  self.log.d('exiting')
  while #modals > 0 do exit(self) end
  if windowSelected then
    self.log.i('focusing', windowSelected)
    --    if toFocus:application():bundleID()~='com.apple.finder' then
    --    toFocus:focus()
    --    else
    timer.doAfter(0.25,
      function()
        if self.__onWindowSelected then
          self.__onWindowSelected(windowSelected)
        else
...
enter = function(self, hints)
  ...
  elseif hints[1] then
    ...
    if modals[#modals] then
      self.log.vf('exit modal %d', #modals)
      modals[#modals].modal:exit()
    end
    ...
    for c, t in pairs(hints) do
      if type(c) == 'string' and #c == 1 then
        modal:bind({}, c, function()
          updateHighlights(self.ui, hints, t, true)
          enter(self, t)
        end)
        modal:bind({ self.ui.closeModeModifier }, c, function()
          updateHighlights(self.ui, hints, t, true)
          enter(self, t)
        end)
        modal:bind({ self.ui.minimizeModeModifier }, c,
          function()
            updateHighlights(self.ui, hints, t, true)
            enter(self, t)
          end)
...
local function setThumbnail (w, screenFrame, thumbnails, titles, ui, bg)
  ...
  local br = geom.copy(wframe):seth(hintHeight):setw(hintWidth + hintHeight + padding * 4):setcenter(wframe.center):fit(
    screenFrame)
...
local function showExpose (self, windows, animate, alt_algo)
  ...
  if not self.running then
    self.log.i('instance not running, cannot show expose')
    return
  end
...
local function getApplicationWindows ()
  ...
  if not a then
    log.w('cannot get active application')
    return
  end
  local r = {}
...
local function bgFitWindows ()
  ...
  for self in pairs(activeInstances) do
    ...
    if self.dirty then
      for _, screen in pairs(self.screens) do
        if screen.dirty then
          fitWindows(self, screen, iters)
          rep = rep or screen.dirty
        end
    ...
    if DEBUG then
      print(math.floor((timer.secondsSinceEpoch() - DEBUG_TIME) / iters * 1000) ..
        'ms per iteration - ' .. iters .. ' total')
    end
  end
...
local function startBgFitWindows (ui)
  if activeInstance and not ui.fitWindowsInBackground then
    bgRepeats = 2
    return bgFitTimer:start(0.05)
  end
...
local function windowAllowed (self, win, appname, screen)
  ...
  w.hintrect = drawing.rectangle(f):setFill(true):setFillColor(ui.highlightColor)
      :setStroke(true):setStrokeWidth(min(ui.textSize / 10, ui.highlightThumbnailStrokeWidth)):setStrokeColor(ui
        .highlightColor)
...
local function resume (self)
  if not activeInstances[self] then
    self.log.i('instance stopped, ignoring resume')
    return self
  end
  -- subscribe
  for _, s in pairs(self.screens) do
...
local function pause (self)
  if not activeInstances[self] then
    self.log.i('instance stopped, ignoring pause')
    return self
  end
  -- unsubscribe
...
local function makeScreens (self)
  ...
  for _, scr in ipairs(hsscreens) do -- populate current screens
    ...
    if sid and sname then
      local wf = windowfilter.copy(self.wf, 'wf-' .. self.__name .. '-' .. sid):setDefaultFilter {}
          :setOverrideFilter { visible = true, currentSpace = true, allowScreens = sid, activeApplication = activeApplication }
          :keepActive()
      screens[sid] = {
        name = sname,
        wf = wf,
        ...
        frame = sframe,
        totalOriginalArea = 0,
        thumbnails = self.ui.showThumbnails,
        edge = geom.new(0, 0),
  ...
  if not next(screens) then
    self.log.w 'no valid screens found'
  ...
  if self.ui.includeNonVisible then
    do -- hidden windows strip
      ...
      screens.inv = {
        ...
        edge = edge,
        pos = pos,
        bg = drawing.rectangle(invf):setFill(true):setFillColor(self.ui.nonVisibleStripBackgroundColor):setBehavior(
          BEHAVIOR)
      }
    ...
    for sid, screen in pairs(screens) do -- other spaces strip
      if not screen.isStrip then
        ...
        local wf = windowfilter.copy(self.wf, 'wf-' .. self.__name .. '-o' .. sid):setDefaultFilter {}
            :setOverrideFilter { visible = true, currentSpace = false, allowScreens = sid, activeApplication = activeApplication }
            :keepActive()
        ...
        oscreens['o' .. sid] = {
          ...
          edge = edge,
          pos = pos,
          bg = drawing.rectangle(othf):setFill(true):setFillColor(self.ui.otherSpacesStripBackgroundColor):setBehavior(
            BEHAVIOR)
        }
...
local function start (self)
  if activeInstances[self] then
    self.log.i('instance already started, ignoring')
    return self
  end
...
local function stop (self)
  if not activeInstances[self] then
    self.log.i('instance already stopped, ignoring')
    return self
  end
  pause(self)
  deleteScreens(self)
...
function expose.new (wf, uiPrefs, logname, loglevel)
  ...
  local self = setmetatable({ screens = {}, windows = {}, __name = logname or 'expose' },
    { __index = expose, __gc = stop })
  ...
  if wf == nil then
    self.log.i('new expose instance, using default windowfilter')
    wf = windowfilter.default
  else
    self.log.i('new expose instance using windowfilter instance')
    wf = windowfilter.new(wf)
  end
  --uiPrefs
  self.ui = setmetatable({}, {
    __newindex = function(t, k, v)
      rawset(self.ui, k, getColor(v))
      if not inUiPrefs then return setUiPrefs(self) end
    end,
    __index = function(t, k) return getColor(uiGlobal[k]) end,
  })
  for k, v in pairs(uiPrefs) do rawset(self.ui, k, getColor(v)) end
  setUiPrefs(self)
  --  local wfLogLevel=windowfilter.getLogLevel()
  --  windowfilter.setLogLevel('warning')
...
---@param onWindowSelected function callback run on window selection,
--- takes the selected window object as input, if this was set, then the window focus will not run
function expose:setCallback (onWindowSelected)
  self.__onWindowSelected = onWindowSelected
end
