...
function M:report_frontmost_window() --luacheck: no self
    ...
    local rect_string = string.format("[%.0f,%.0f>%.0f,%.0f]", rect.x1, rect.y1, rect.x2, rect.y2)

    local unit_rect = window:screen():toUnitRect(rect)
    local unit_rect_string =
        string.format(
        "[%.0f,%.0f>%.0f,%.0f]",
        unit_rect.x1 * 100,
        unit_rect.y1 * 100,
...
function M:report_screens() --luacheck: no self
    local screens =
        ...
        function(_, screen)
            return {name = screen:name(), id = screen:id()}
        end
    )
...
M.chooser = hs.chooser.new(completionFn):choices(choicesFn):searchSubText(true)
...
function M:activeLayouts() -- :string
    ...
    function active_layouts:tostring()
        return tostring(self)
    end -- luacheck: no redefined
